 ...
 int decode_packet(int *got_frame, int cached)
{
    ...
    *got_frame = 0;
    if (pkt.stream_index == video_stream_idx) {
        ...
        if (ret < 0) {
            LOGI("Error decoding video frame (%s)\n", av_err2str(ret));
            return ret;
        }
        if (*got_frame) {
            if (frame->width != width || frame->height != height ||
                ...
                LOGI("Error: Width, height and pixel format have to be "
            ...
            LOGI("video_frame%s n:%d coded_n:%d\n",
                   cached ? "(cached)" : "",
        ...
        if (ret < 0) {
            LOGI("Error decoding audio frame (%s)\n", av_err2str(ret));
            return ret;
        }
                                      ...
                                      enum AVSampleFormat sample_fmt)
{
    ...
    *fmt = NULL;
    ...
    LOGI("sample format %s is not supported as output format\n",
...
int demuxAndDecodeForFFmpeg (char *inputPath, char *outputAudioPath,char *outputVideoPath)
{
    ...
    src_filename = inputPath;
    video_dst_filename = outputAudioPath;
    audio_dst_filename = outputVideoPath;
    ...
    pkt.size = 0;
    if (video_stream)
        LOGI("Demuxing video from file '%s' into '%s'\n", src_filename, video_dst_filename);
    if (audio_stream)
        LOGI("Demuxing audio from file '%s' into '%s'\n", src_filename, audio_dst_filename);
    ...
    if (audio_stream) {
        ...
        if (av_sample_fmt_is_planar(sfmt)) {
            ...
            LOGI("Warning: the sample format the decoder produced is planar "
